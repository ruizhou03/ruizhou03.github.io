...
- release marker: `20260801p7j`
- build: `2026.08.01.phase7-j`
...
## VoiceOver 分段选择状态阶段 7J 证据（2026-08-01）

- 用户开启真实 macOS VoiceOver 后，人工验收前置检查发现设置页互斥按钮只用
  `selected` / `active` 视觉类表达当前选项，辅助技术无法知道“单机、普通、中、
  同队进贡关、25 秒、创建房间”等哪一项已选；该问题按新发现 P1 阻断人工验收。
- 玩法模式、难度、手牌大小、同队进贡、出牌时间和联机方式均增加可访问组名；
  所有选项统一维护 `aria-pressed`，每组始终恰好一个 true，并与视觉类使用同一
  状态同步函数，避免只修初始 HTML 后在切换时再次漂移。
- 系统 Safari 26.5.2 的真实浏览器回归依次切换普通→新手→普通、单机→联机、
  创建→加入→创建、联机→单机；7 个组在初始和往返切换后均为唯一 pressed、
  唯一视觉选中且二者一致。该次回归同时通过 27 张手牌、机器人图标、键盘选牌、
  调整顺序、dialog、44×44px 工具栏、倒计时、托管和结算摘要。
- 以上是自动语义前置证据；VoiceOver 实际发音和可操作性仍由用户在生产 `p7j`
  上真人确认，不能以 WebDriver 结果代替。
