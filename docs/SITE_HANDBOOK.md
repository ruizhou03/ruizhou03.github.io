...
- 登录：邮件验证码为 8 位随机数，摘要存 Redis、10 分钟到期、成功后原子销毁；按 IP 与邮箱限制发送和猜测。也可用已验签的 Google ID token 登录，并按 Google `sub` 绑定。本站密码入口已全部关闭，不再创建或使用密码；生产账号记录中的历史密码哈希已清除。SiteAuth 使用无状态 JWT，支持多设备。
