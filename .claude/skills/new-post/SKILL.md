...
- 学习笔记：`_notes/<exam>/`（gre, toefl, tutoring）、`_notes/study/<course>/`；课程测评 `_notes/course-reviews/` 也走这套（`material_type: "课程测评"`），不另立 schema
...
- 大型考试 → `_notes/<exam>/<slug>.md`（gre, toefl, tutoring）
