...
published: true
keywords: ["LaTeX 快捷键", "LaTeX 自定义命令", "newcommand", "renewcommand", "commands.tex", "LaTeX 宏定义", "数学符号简写", "mathbb 数集", "自动伸缩括号", "left right 括号", "可选参数 默认值", "DeclareMathOperator", "算子定义", "草稿 TODO 宏", "LaTeX macros", "amsmath", "aligned cases 简写", "写公式提速", "LaTeX 偷懒", "Latex 快捷键", "Overleaf", "VSCode LaTeX", "LaTeX 笔记 PhD", "经济学 LaTeX", "数学排版 LaTeX", "amsmath 宏包", "xcolor textcolor", "ForestGreen 颜色", "LaTeX 命令冲突", "single letter macros", "renew 命令冲突", "LaTeX 写作效率", "macros 写论文", "LaTeX 模板 PhD"]
---
...
完整的 LaTeX 快捷键大全放在下一个章节，方便有需要的读者朋友自取。更具体的解释在更下一个章节，有兴趣的朋友可以继续阅读。
...
这里的 `[1]` 表示这个命令接收 **1 个参数**。调用时只需要写 `\pr{\frac{1}{2}}`，LaTeX 就会把它展开成 `\left(\frac{1}{2}\right)`，完美生成自适应大小的 $\left(\frac{1}{2}\right)$。
...
<span style="color:#888;font-size:0.9em;">（注：这里的 `\!` 是一个小小的负间距，用来微调 $\mathbb{E}$ 和括号之间的距离，这也属于一点强迫症的修饰。）</span>
