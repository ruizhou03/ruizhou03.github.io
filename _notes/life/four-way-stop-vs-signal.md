...
这里最重要的不是记住一串参数，而是模型没有把答案提前写死：four-way stop 允许不冲突的对向车辆一起走，也会在四面都有车时承受更多协调时间；红绿灯会付出清场和队首起步的成本，但绿灯中段可以连续放车。每种流量重复运行 40 次，取平均结果。具体设定和重复次数检查放在文末，正文先来看发生了什么。
