...
- **5 GHz**：波长 6 cm，速率快、信道多（\$20+$ 个）——但隔一堵承重墙就大幅衰减
...
电磁波在真空中的速度是固定的光速 $c \approx 3 \times 10^8\,\text{m/s}$。频率 $f$ 和波长 $\lambda$ 的关系是：

> $$\lambda = c / f$$

代入：

- 2.4 GHz WiFi → $\lambda \approx$ **$12.5\,$cm**
- 5 GHz WiFi → $\lambda \approx$ **$6\,$cm**
- 6 GHz WiFi → $\lambda \approx$ **$5\,$cm**
- FM 广播（100 MHz） → $\lambda \approx 3\,$m
- 4G LTE（700 MHz 频段） → $\lambda \approx 43\,$cm
...
<p class="img-caption"><strong>dB 是对数刻度</strong>：每 $+10$ dB 信号强度衰减 10 倍、$+20$ dB 衰减 100 倍。一堵混凝土承重墙（5G $\approx 18$ dB）就能把信号衰减到原来的 $1/63$。两堵承重墙叠加 36 dB，相当于 $1/4000$——这就是为什么穿两堵墙基本就什么都不剩了。<strong>5 GHz 的衰减普遍是 2.4 GHz 的 $1.5\text{-}2$ 倍</strong>。</p>
...
5 GHz 频段则有 \$500+\,\text{MHz}$ 可用带宽，能切出 25 个左右非重叠信道（不同国家略有差别）。
...
<p class="img-caption">2.4 GHz 像一条只有 3 个车道的小路，整栋楼几百户人家都在抢；5 GHz 像 25 车道的高速。这就是为什么 2.4G 信号看着满格、实际下载速率却慢——不是信号弱，是<strong>车道堵</strong>。</p>
...
- 缺点：穿透更弱（波长更短）；只有近 $2\text{-}3$ 年的新设备支持
...
- **抬高**：离地至少 1 m，最好 $1.2\text{-}1.8$ m。WiFi 主辐射方向是水平的，离地太低被家具挡死
...
**误区 1：「路由器越贵穿墙越强」**——错。家用路由器最大发射功率被 FCC / 工信部限制（2.4 GHz 通常 $100\,\text{mW} = 20\,\text{dBm}$，5 GHz $200\,\text{mW} = 23\,\text{dBm}$），所有合规产品基本一样。贵的差别在天线设计、芯片性能、并发能力。
