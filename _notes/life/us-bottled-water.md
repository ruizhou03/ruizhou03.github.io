---
layout: post
title: "美国超市的 Drinking / Purified / Spring / Mineral Water 到底有什么区别？"
date: 2026-03-29
main_category: "生活攻略"
sub_category: "生活之问"
extra_categories:
  - 菜谱
permalink: "/life/us-bottled-water"
keywords: ["美国瓶装水区别", "美国超市买水", "Drinking Water", "Purified Water 纯净水", "Spring Water 泉水", "Mineral Water 矿泉水", "Distilled Water 蒸馏水", "Sparkling Water 气泡水", "Alkaline Water 碱性水", "Artesian Water", "Electrolyte Water", "Aquafina", "Dasani", "Fiji 水", "Poland Spring", "FDA 瓶装水分类", "TDS 总溶解固体", "水的硬度", "瓶装水怎么选", "美国喝什么水", "美国瓶状水"]
---

# 1. 问题

美国超市的瓶装水货架是个迷宫——同样是 24 瓶装的水，标签上写着 Drinking Water、Purified Water、Spring Water、Mineral Water、Distilled Water、Sparkling、Alkaline、Artesian、Electrolyte……价格能差 $5$-$10$ 倍。看上去都是无色透明的水，到底差在哪？是营销噱头还是真的不同？该选哪个？

# 2. 结论先行

**美国 FDA 法规（21 CFR 165.110）严格规定了至少 8 类瓶装水的法定标签**——这些词不是营销，每一个都对应特定的水源、工艺、矿物含量要求。**看懂标签就能省下大把钱、不被概念忽悠**。

最重要的几条速查：

| 标签 | 是什么 | 一句话 |
|---|---|---|
| **Drinking Water / Purified Water** | 大多是市政自来水深度处理（RO + 除氯 + 杀菌） | 最便宜的瓶装水；和“凉白开装瓶”差不多 |
| **Distilled Water** | 蒸馏水，$\text{TDS} \approx 0$ | 喝起来“空”；主要用于加湿器 / 熨斗 / 婴儿奶粉 |
| **Spring Water** | 必须来自天然涌出的泉源 | 保留部分天然矿物，是最常见的“喝着有味儿”的瓶装水 |
| **Mineral Water** | 地下水源 + $\text{TDS} \geq 250$ ppm | 矿物显著、口感重；FDA 强制 $250+$ ppm |
| **Artesian Water** | 来自承压含水层的井（自压涌出） | “Fiji 水”那种；噱头 $>$ 实质 |
| **Sparkling Water** | 含 $\text{CO}_2$ 气，三种：天然带气 / 充气矿泉水 / 充气市政水 | La Croix、Perrier、Topo Chico 实际差很多 |
| **Alkaline Water** | pH $8$-$10$，电解或加碱性矿物 | “抗氧化抗衰老”宣称缺乏循证支持，**当普通水喝就好** |
| **Electrolyte / “Enhanced” Water** | 添加钠 / 钾 / 镁 / 钙 | Smartwater 这类；适合大量出汗补给 |

> 美国市场最大的一个 **认知错位**：Aquafina、Dasani 这两个销量最大的品牌都是 **市政自来水 → RO 处理后的 Purified Water**，本质上和你家自来水煮沸滤过差不多——只是装在塑料瓶里、Logo 漂亮。Spring Water（Poland Spring、Arrowhead）才是真正“从地下抽出来”的。

# 3. 科学原理

## 3.1 FDA 法定分类树

美国 FDA 21 CFR 165.110 把瓶装水分成几大类，每类有强制定义：

<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 480" style="max-width:100%;height:auto;display:block;margin:1.2em auto;">
  <text x="360" y="22" text-anchor="middle" font-size="14" fill="#333" font-weight="600">FDA 瓶装水法定分类（21 CFR 165.110）</text>
  <rect x="270" y="50" width="180" height="50" fill="#3498db" stroke="#2980b9" stroke-width="2" rx="6"/>
  <text x="360" y="72" text-anchor="middle" fill="#fff" font-size="13" font-weight="700">Bottled Water</text>
  <text x="360" y="90" text-anchor="middle" fill="#fff" font-size="11">（瓶装水总称）</text>
  <line x1="120" y1="135" x2="600" y2="135" stroke="#888" stroke-width="1"/>
  <line x1="360" y1="100" x2="360" y2="135" stroke="#888" stroke-width="1"/>
  <line x1="120" y1="135" x2="120" y2="150" stroke="#888" stroke-width="1"/>
  <line x1="280" y1="135" x2="280" y2="150" stroke="#888" stroke-width="1"/>
  <line x1="440" y1="135" x2="440" y2="150" stroke="#888" stroke-width="1"/>
  <line x1="600" y1="135" x2="600" y2="150" stroke="#888" stroke-width="1"/>
  <g transform="translate(40,150)">
    <rect x="0" y="0" width="160" height="280" fill="#fff5e6" stroke="#e67e22" stroke-width="1.5" rx="6"/>
    <text x="80" y="22" text-anchor="middle" font-size="12" fill="#d35400" font-weight="700">按来源</text>
    <text x="80" y="36" text-anchor="middle" font-size="10" fill="#666">（自然涌出）</text>
    <text x="10" y="60" font-size="11" fill="#333" font-weight="600">Spring Water</text>
    <text x="10" y="76" font-size="9" fill="#666">天然涌出地表的泉</text>
    <text x="10" y="100" font-size="11" fill="#333" font-weight="600">Artesian Water</text>
    <text x="10" y="116" font-size="9" fill="#666">承压含水层井</text>
    <text x="10" y="140" font-size="11" fill="#333" font-weight="600">Well Water</text>
    <text x="10" y="156" font-size="9" fill="#666">普通井水</text>
    <text x="10" y="180" font-size="11" fill="#333" font-weight="600">Mineral Water</text>
    <text x="10" y="196" font-size="9" fill="#666">地下水 + TDS ≥ 250</text>
    <text x="10" y="220" font-size="11" fill="#333" font-weight="600">Glacier / Iceberg</text>
    <text x="10" y="236" font-size="9" fill="#666">冰川 / 冰山融水</text>
    <text x="10" y="262" font-size="9" fill="#888" font-style="italic">→ 保留天然矿物</text>
  </g>
  <g transform="translate(200,150)">
    <rect x="0" y="0" width="160" height="280" fill="#e8f5fe" stroke="#3498db" stroke-width="1.5" rx="6"/>
    <text x="80" y="22" text-anchor="middle" font-size="12" fill="#2980b9" font-weight="700">按工艺</text>
    <text x="80" y="36" text-anchor="middle" font-size="10" fill="#666">（处理过的）</text>
    <text x="10" y="60" font-size="11" fill="#333" font-weight="600">Purified Water</text>
    <text x="10" y="76" font-size="9" fill="#666">RO / 蒸馏 / 去离子</text>
    <text x="10" y="92" font-size="9" fill="#666">TDS ≤ 10 ppm</text>
    <text x="10" y="118" font-size="11" fill="#333" font-weight="600">Distilled Water</text>
    <text x="10" y="134" font-size="9" fill="#666">蒸馏，TDS 极低</text>
    <text x="10" y="160" font-size="11" fill="#333" font-weight="600">Drinking Water</text>
    <text x="10" y="176" font-size="9" fill="#666">符合 EPA 饮用水标准</text>
    <text x="10" y="192" font-size="9" fill="#666">通常市政水深度处理</text>
    <text x="10" y="218" font-size="11" fill="#333" font-weight="600">Deionized Water</text>
    <text x="10" y="234" font-size="9" fill="#666">离子交换，TDS 极低</text>
    <text x="10" y="262" font-size="9" fill="#888" font-style="italic">→ 矿物质几乎被去除</text>
  </g>
  <g transform="translate(360,150)">
    <rect x="0" y="0" width="160" height="280" fill="#e8f8e8" stroke="#27ae60" stroke-width="1.5" rx="6"/>
    <text x="80" y="22" text-anchor="middle" font-size="12" fill="#27ae60" font-weight="700">按形态</text>
    <text x="80" y="36" text-anchor="middle" font-size="10" fill="#666">（含 / 不含气）</text>
    <text x="10" y="60" font-size="11" fill="#333" font-weight="600">Still Water</text>
    <text x="10" y="76" font-size="9" fill="#666">不含气泡（默认）</text>
    <text x="10" y="100" font-size="11" fill="#333" font-weight="600">Sparkling Water</text>
    <text x="10" y="116" font-size="9" fill="#666">含 CO₂ 气</text>
    <text x="10" y="140" font-size="10" fill="#666" font-style="italic">  ├ Sparkling Mineral</text>
    <text x="10" y="156" font-size="9" fill="#666">  天然矿泉 + 气</text>
    <text x="10" y="174" font-size="10" fill="#666" font-style="italic">  ├ Seltzer / Club Soda</text>
    <text x="10" y="190" font-size="9" fill="#666">  市政水充气</text>
    <text x="10" y="208" font-size="10" fill="#666" font-style="italic">  └ Tonic Water</text>
    <text x="10" y="224" font-size="9" fill="#666">  含奎宁，苦味</text>
    <text x="10" y="262" font-size="9" fill="#888" font-style="italic">→ 标签字眼差很多</text>
  </g>
  <g transform="translate(520,150)">
    <rect x="0" y="0" width="160" height="280" fill="#fde8e8" stroke="#c0392b" stroke-width="1.5" rx="6"/>
    <text x="80" y="22" text-anchor="middle" font-size="12" fill="#c0392b" font-weight="700">按"附加价值"</text>
    <text x="80" y="36" text-anchor="middle" font-size="10" fill="#666">（多数为营销）</text>
    <text x="10" y="60" font-size="11" fill="#333" font-weight="600">Alkaline Water</text>
    <text x="10" y="76" font-size="9" fill="#666">pH 8-10</text>
    <text x="10" y="92" font-size="9" fill="#666">电解或加碱</text>
    <text x="10" y="118" font-size="11" fill="#333" font-weight="600">Electrolyte Water</text>
    <text x="10" y="134" font-size="9" fill="#666">+ Na/K/Mg/Ca</text>
    <text x="10" y="160" font-size="11" fill="#333" font-weight="600">Vitamin Water</text>
    <text x="10" y="176" font-size="9" fill="#666">+ 维生素 + 糖</text>
    <text x="10" y="202" font-size="11" fill="#333" font-weight="600">Hydrogen Water</text>
    <text x="10" y="218" font-size="9" fill="#666">+ H₂ 气</text>
    <text x="10" y="262" font-size="9" fill="#c0392b" font-style="italic">→ 多数 ≠ 健康收益</text>
  </g>
</svg>
<p class="img-caption">这四列分类**互相不冲突**——一瓶水可以同时属于多个标签。比如 Perrier = Sparkling + Mineral Water + 来自法国天然泉源；Smartwater = Distilled（工艺）+ Electrolyte Enhanced（添加）。看标签时不要找"它属于哪一类"，要找"它符合哪些限定词"。</p>

## 3.2 关键术语：TDS、pH、硬度、电解质

读瓶装水标签或营销话术，会反复看到这几个词。一次性讲清楚：

**TDS（Total Dissolved Solids，总溶解固体）**

- 单位：ppm（mg/L）
- 测量水里所有溶解的矿物质、盐、有机物的总量
- **不直接等于硬度**——硬度只算钙镁；TDS 还包括钠、钾、氯化物、硫酸根、碳酸氢根等
- 范围：
  - Purified Water：$< 10$ ppm（FDA 强制）
  - 普通自来水：$50$-$500$ ppm
  - Spring Water：$50$-$300$ ppm
  - **Mineral Water（FDA 定义）：$\geq 250$ ppm**
  - 海水：$\sim 35{,}000$ ppm

**pH**

- $0$-$14$ 的对数刻度
- 中性 $= 7$；$< 7$ 酸；$> 7$ 碱
- 多数自来水：$6.5$-$8.5$
- “Alkaline Water”：通常 $8$-$10$

> 注意：人胃里的 pH 是 **$1$-$2$（强酸）**——所以喝下去的“碱性水”几秒钟内就被胃酸中和。“碱性水改变体液 pH”在生理学上不可能。

**硬度（Hardness）**

- 钙 + 镁的总量，以 mg/L $\text{CaCO}_3$ 计
- 见 [电热水壶水垢能喝吗？](/life/kettle-scale) 详细讨论

**电解质（Electrolytes）**

- 在水里能解离成离子的物质
- 最常见：$\text{Na}^+$（钠）、$\text{K}^+$（钾）、$\text{Mg}^{2+}$（镁）、$\text{Ca}^{2+}$（钙）、$\text{Cl}^-$（氯）、$\text{HCO}_3^-$（碳酸氢根）
- 体液平衡需要电解质——但日常饮食中几乎所有食物都含丰富电解质
- “Electrolyte Water” 营销点是大量出汗后补充；普通日常喝不需要刻意选这种

## 3.3 来源 vs 处理：水的“血统”和“工艺”

读懂瓶装水标签需要分清两件事：

**来源（Source）**：水从哪里来

- **市政水（Municipal）**：来自城市自来水系统（这就是 Aquafina / Dasani 的真相）
- **天然泉（Spring）**：天然涌出地表的泉水
- **承压井（Artesian Well）**：钻到承压含水层、自动喷涌的井
- **普通井（Well）**：抽地下水
- **冰川 / 冰山（Glacier / Iceberg）**：融化的冰川或漂浮冰山
- **海洋（Ocean）**：海水脱盐（少见）

**处理（Processing）**：水做了什么

- **逆渗透（RO，Reverse Osmosis）**：高压把水推过半透膜，过滤掉几乎所有杂质（包括矿物）
- **蒸馏（Distillation）**：加热蒸发，再凝结——和 RO 类似，矿物全去除
- **去离子（Deionization）**：离子交换树脂去除带电粒子
- **超滤（Ultrafiltration）**：膜过滤，孔径稍大于 RO
- **臭氧消毒（Ozonation）**：用 $\text{O}_3$ 灭菌（替代氯）
- **紫外线消毒（UV）**：辐射杀菌
- **微滤 + 加矿物（Mineralization）**：去除杂质后人工加回钙 / 镁等矿物

> **关键洞察**：标签上的产品名不直接告诉你来源。**找 “Source:” 这一行**——任何瓶装水都必须标注。如果写 “Public Water Supply” 或某个城市名，那就是市政水；如果写某个 spring 的名字，那就是天然泉源。

## 3.4 美国超市常见品牌大对照

按价格 / 类型把美国市场最常见的瓶装水梳理一遍：

| 品牌 | 类型 | 来源 | TDS 范围 | pH | 价格档（24×16.9oz） |
|---|---|---|---|---|---|
| **Generic / Store Brand**（Kirkland、Great Value） | Purified | 市政水 → RO | $< 10$ | $\sim 7$ | $3-5 |
| **Aquafina**（百事） | Purified | 多个市政来源 → RO | $< 10$ | $\sim 6$ | $5-7 |
| **Dasani**（可口可乐） | Purified + 加 minerals | 市政 → RO + 重新加 K/Mg/盐 | $\sim 30$ | $\sim 5.6$ | $5-7 |
| **Nestle Pure Life** | Purified | 市政 + spring 混合 | $\sim 80$-$200$ | $\sim 7$ | $5-7 |
| **Smartwater**（可口可乐） | Vapor distilled + 电解质 | 市政 → 蒸馏 + 加 K/Mg/Ca | $\sim 30$ | $\sim 7$ | $7-10 |
| **Poland Spring**（Nestle） | Spring | 缅因州天然泉 | $\sim 40$-$100$ | $\sim 7$ | $6-8 |
| **Arrowhead**（Nestle） | Spring | 加州山泉 | $\sim 50$-$150$ | $\sim 7$ | $6-8 |
| **Crystal Geyser** | Spring | 多个州（加州、田纳西等） | $\sim 80$-$200$ | $\sim 7$ | $5-7 |
| **Mountain Valley** | Spring | 阿肯色州 Hot Springs | $\sim 220$ | $\sim 7.8$ | $15-25 |
| **Evian**（达能） | Mineral / Spring（FDA 标 Mineral） | 法国 Évian 阿尔卑斯 | $\sim 309$ | $\sim 7.2$ | $20-30 |
| **Fiji** | Artesian | 斐济 Yaqara 山谷承压含水层 | $\sim 222$ | $\sim 7.7$ | $25-35 |
| **Voss** | Artesian | 挪威或得克萨斯 | $\sim 44$（Voss Norway） | $\sim 6$ | $25-40 |
| **Perrier**（雀巢） | Sparkling Mineral | 法国 Vergèze 天然带气泉 | $\sim 475$ | $\sim 5.5$ | $20-30 |
| **San Pellegrino**（雀巢） | Sparkling Mineral | 意大利 Bergamo | $\sim 937$ | $\sim 7.7$ | $20-30 |
| **Topo Chico**（可口可乐） | Sparkling Mineral | 墨西哥 Cerro del Topo Chico | $\sim 625$ | $\sim 6$ | $25-35 |
| **La Croix** | Seltzer（充气市政水）+ 香精 | 美国市政 + $\text{CO}_2$ | $< 10$ | $\sim 5$ | $5-7 |
| **Bubly**（百事） | Seltzer + 香精 | 同 La Croix | $< 10$ | $\sim 5$ | $5-7 |
| **Spindrift** | Seltzer + 真水果汁 | 市政 + $\text{CO}_2$ + 浓缩果汁 | $< 50$ | $\sim 3.8$ | $7-10 |
| **Liquid Death**（铝罐） | Mountain water 或 Sparkling | 阿尔卑斯山 | $\sim 150$ | $\sim 7$ | $15-20 |
| **Essentia** | Alkaline (ionized) | 市政 → 电解 → pH $9.5$ | $\sim 50$ | $\sim 9.5$ | $10-15 |
| **Core** | Purified + 微量电解质 | 市政 → RO + 加少量电解质 | $\sim 10$ | $\sim 7.4$ | $7-10 |
| **Aquahydrate** | Alkaline + 电解质 | 市政 → RO → 加碱 + 电解质 | $\sim 30$ | $\sim 9$ | $10-15 |
| **Distilled Water**（无品牌通货） | Distilled | 蒸馏 | $\sim 0$ | $\sim 7$ | $1-2 / 1 gallon |

**看完这张表的几个 takeaway**：

1. **Aquafina 和 Dasani 本质就是处理过的自来水**——百事和可口可乐这两个品牌占美国瓶装水市场半壁江山，价格却比超市自有品牌的 Purified Water 贵 $50\%$
2. **Voss 和 Fiji 价格悬殊但水本身平平**——Voss 主打那个圆柱玻璃瓶设计，Fiji 主打“原始热带”叙事；TDS 不算高
3. **真正“特别”的水主要在 Mineral Water 那一档**——Pellegrino、Perrier 的 TDS 是 Aquafina 的 $50$-$100$ 倍，喝起来明显有矿物感
4. **Sparkling Water 三大类要分清**——La Croix（市政 + 充气 + 香精，不算“矿泉”）、Topo Chico / Pellegrino（天然带气矿泉水）、Perrier（天然矿泉水 + 增气）
5. **Alkaline Water（Essentia / Aquahydrate）**：FDA 不允许它们宣称“治病”，但允许你为高 pH 多付钱

# 4. 实践建议

## 4.1 按场景该选哪种

**🥤 日常解渴**

最便宜的 Generic / Store Brand Purified Water 就够了。Aquafina / Dasani 没有比 Kirkland Signature 健康，只是品牌溢价。

**☕ 泡茶 / 煮咖啡**

- 泡茶：Spring Water（Poland Spring、Arrowhead），硬度适中
- 手冲咖啡：避开 Distilled / Purified Water（太软，萃取过度）；Spring Water 或专门的咖啡水（如 Third Wave Water 矿物盐 + Distilled 自配）
- 详见 [做饭用自来水还是瓶装纯净水？](/life/cooking-water)

**👶 婴儿冲奶粉**

- Distilled Water 或 标注 “Nursery Water” / “Infant Water” 的产品
- **不要用 Mineral Water**（钠 / 钙过高，婴儿肾负担不起）
- **不要用 Spring Water**（矿物含量不稳定）
- 美国 CDC + FDA 推荐：Distilled 或 Purified Water 煮沸冷却到 $70\,$℃ 以上冲泡

**🏃 大量出汗 / 运动后**

- 中低强度（普通锻炼 $< 1$ 小时）：普通水 + 正常吃饭就够了
- 高强度 / 长时间（跑步 $> 1.5$ 小时、徒步、热瑜伽）：Electrolyte Water（Smartwater 不算够，需要更高电解质浓度）或正经的运动饮料（Gatorade、Nuun 泡腾片、LMNT 包）
- “Alkaline Water 提高运动表现”——证据非常薄弱，正常水 + 电解质就足够

**🍷 配葡萄酒 / 品鉴 / 高端餐桌**

- Still Mineral Water（Evian、Mountain Valley）或低气泡 Sparkling（San Pellegrino）
- 餐厅常给“Still or Sparkling?”——Still 配葡萄酒不影响品鉴；Sparkling 适合配油腻菜清口
- 不要选高 TDS 的 Pellegrino 配淡白葡萄酒（矿物味会盖过酒）

**💧 加湿器 / 熨斗 / 鱼缸（短期）/ 电池**

- **必须 Distilled Water**——任何含矿物的水都会在加湿器 / 熨斗里结垢，损坏设备
- 鱼缸要用 dechlorinated 水（去氯），但 Distilled 缺矿物对鱼也不好；要加专门的水族盐
- 汽车铅酸电池 / CPAP 呼吸机：必须 Distilled

**🍸 调酒 / 鸡尾酒**

- Sparkling Water：选低钠、低矿物的（Topo Chico、Perrier 偏矿；La Croix、Bubly 几乎无矿）
- Tonic Water 和 Club Soda 不一样——Tonic 含奎宁 + 糖，专配金酒；Club Soda 含微量矿盐

## 4.2 看懂标签的几个要点

每瓶水都必须标注：

1. **Source（来源）**——这是关键信息
   - “Public Water Supply” / 城市名 → 市政水
   - 某个 spring 名字 → 天然泉
   - “Artesian Well” + 地名 → 承压井
2. **Treatment（处理）**——告诉你工艺
   - “Reverse Osmosis”（RO）
   - “Distilled” / “Vapor Distilled”
   - “Ozonated” / “UV Disinfection”
3. **Nutrition Facts**——电解质含量
   - 每 240 ml（8 fl oz）的钠、钙、镁、钾
   - **看钠**：高血压人群避开 $> 25$ mg / $8$ fl oz 的水
   - **看钙镁**：硬水补矿，软水适合敏感场景

> **小心一种营销陷阱**：瓶子上印 “From the Pristine Mountain Springs of XXX” 字样，但 Source 那行写的是 “Municipal Source from XXX City”——前者是营销文案，后者是法定声明，**法定声明才作数**。

## 4.3 几个误区

**误区 1：「Purified Water 比自来水更安全」**

不一定。NRDC 和 EWG 多次研究发现部分 Purified Water 的某些指标（包括微塑料、邻苯二甲酸盐）甚至比 EPA 自来水标准还差。塑料瓶在阳光 / 高温下储存会渗出微量化学物。**一杯煮沸的家用 RO 净水器水质量并不输 Aquafina**。

**误区 2：「Alkaline Water 抗氧化、改变体液 pH、抗癌」**

证据基本都是体外或小样本研究，且大多由产品公司自己资助。Mayo Clinic、AHA 的官方立场是：**没有可靠证据显示 Alkaline Water 比普通水有任何健康优势**。胃酸（pH 1-2）会瞬间中和任何 pH 9 的水——所以“改变体液 pH”在生理学上不成立。

**误区 3：「Mineral Water 比 Spring Water 更营养、更健康」**

也错。Mineral Water 的 TDS 高（FDA 强制 $\geq 250$ ppm），含矿多——但**膳食里水的矿物贡献占比不到 $1\%$**，正常吃饭根本不会因为换了 Mineral Water 在矿物摄入上有可观差别。Mineral Water 的真正价值是 **风味（餐桌享受）** 而不是 “更健康”。

**误区 4：「氢水（Hydrogen Water）能抗氧化」**

证据极薄弱，且氢气在水里溶解度极低（饱和也只有 $1.6$ mg/L）+ 开瓶后几分钟就跑光。商业氢水基本是营销。

**误区 5：「冰川水 / 火山水 / 雨林深处的水有特殊能量」**

这是叙事营销不是科学。一瓶水的物理化学性质完全由它的 mineral profile 和处理工艺决定，“水源故事”不会改变水分子的行为。

## 4.4 钱该怎么花

按“性价比 vs 健康收益”排序我会这么花：

1. **第一步：装家用过滤器**（PUR / Brita 滤水壶 $30，或 Aquasana / APEC 台下 RO $200-400）—— 一年成本 $50-150，喝水量最大的家庭场景全覆盖
2. **第二步：买一箱 Generic Spring Water 或 Purified Water**——出门 / 不方便煮水时用，$5-7 / 24 瓶就够
3. **第三步：偶尔买一瓶好的 Mineral Water**（Pellegrino、Mountain Valley）——餐桌享受、配饭，**当饮料对待**
4. **不要常买的**：Aquafina / Dasani（贵 + 是处理过的市政水）、Alkaline Water（无健康收益）、Vitamin Water（含糖 = 软饮料）
5. **特殊场景才买**：Distilled（加湿器 / 熨斗 / 婴儿）、Electrolyte（运动）

# 5. 参考来源

1. **U.S. Food and Drug Administration.** *21 CFR 165.110 - Bottled Water.* Code of Federal Regulations. ——FDA 瓶装水法定分类的官方规定，本文所有“法定”定义的源头。
2. **U.S. Environmental Protection Agency.** *National Primary Drinking Water Regulations.* ——EPA 自来水水质标准，瓶装水以此为最低基准。
3. **International Bottled Water Association.** *IBWA Bottled Water Code of Practice.* ——美国瓶装水行业自律标准，包括 FDA 之外的额外要求。
4. **Natural Resources Defense Council (NRDC).** *Bottled Water: Pure Drink or Pure Hype? (2013 update).* ——NRDC 关于瓶装水水质问题的独立调查报告。
5. **Heil DP.** **Acid-base balance and hydration status following consumption of mineral-based alkaline bottled water.** *Journal of the International Society of Sports Nutrition.* 2010;7:29. ——Alkaline Water 对体液 pH 的影响（结论：影响极有限）。
6. **Mayo Clinic Staff.** *Alkaline water: Better than plain water?* mayoclinic.org ——Mayo Clinic 关于 Alkaline Water 健康宣称的官方立场。
7. **Cohen J, et al.** **Microplastic contamination in bottled water: a systematic review.** *Frontiers in Chemistry.* 2018;6:407. ——瓶装水微塑料含量的系统综述。
