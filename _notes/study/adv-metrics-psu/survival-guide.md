...
- 对“如何在 4 天内组织一个 ROI 优先的复习方案”感兴趣的人。
...
讲义里有一份精确到天的复习节奏：哪天读哪章、哪天做模拟考、哪天彻底不学新东西早睡。事后看执行得不错，比无头苍蝇式复习至少省了一半时间。
...
- 写作时间有限（约 8 天），结构性的错漏几乎肯定存在。发现错误欢迎告诉我。
- 写作过程中我用 Claude 做了大量的格式整理、错排修复和文字润色——它在补充 LaTeX 排版、统一术语、抓 typo 上帮了大忙。但所有 **结构性决策**（章节顺序、tier 系统、tag 等级、考点优先级、应试策略）全部由我自己制定，AI 不知道 Patrik 的命题偏好。
