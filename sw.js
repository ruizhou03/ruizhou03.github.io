...
const DDZ_SAVED_MIGRATION = '/__zircon_migrations__/doudizhu-20260801g8c';
...
async function migrateSavedDoudizhu() {
  try {
    ...
    await cache.put(DDZ_SAVED_MIGRATION, new Response('ok', {
      headers: { 'content-type': 'text/plain', 'x-zircon-migration': '20260801g8c' },
    }));
  } catch (_) {
...
self.addEventListener('message', async (event) => {
  ...
  if (data.type === 'SAVE_OFFLINE') {
    ...
    if (urls.some((rawUrl) => {
      ...
      try {
        const manifestUrl = '/toolbox/guandan/offline-assets.json?v=20260801p7j';
