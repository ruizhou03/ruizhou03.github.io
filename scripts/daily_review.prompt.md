...
2. **跑自动巡检脚本**：`bash scripts/audit/run.sh > /tmp/audit_report.md 2>&1`。一组按日期智能调度的 audit：keywords 漏检 + 图片可发现性与体积 + 后端脉搏（zircon-urge / zircon-comments 公开接口）每天跑；死链巡检每周一加跑；月度内容统计每月 1 号加跑。把 `/tmp/audit_report.md` 完整读进来，作为本次巡检的客观依据。**这份报告里挑出来的具体问题，逐项判断是修是留**：
   ...
   - **🔬 抽检专项**：`spotcheck.py` 会按日期种子从全站随机抽 **10 个**”项目”（建设期高频抽检：每天强制保底 1 game + 1 pdf_archive + 1 lecture_note_pdf_only，其余 7 项纯随机；这三类是建设期重点品类），并对每项给出**类型量身定制的批判性审查清单**。这是每日巡检中**最重头、最深入**的一节——绝不能糊弄：
     - 对每个抽中项**逐条**走清单（不是泛泛说”挺好的”）。每个 checklist item 必须明确给出三种结论之一：① 没问题 ② 有问题（描述 + 建议） ③ 拿不准（写进待办）。
     - **打开文件实际读**：笔记/文章要通读全文；游戏要 grep 出 HTML/JS/CSS 实际看代码结构、在脑中走完一局完整的游戏流程；PDF 存档要 `grep -r “<pdf路径>” _notes/ docs/` 看引用情况、必要时打开 PDF 取首页摘要。
     - **小而无争议的问题直接修**（错别字、明显坏链、缺 caption、缺 keywords、front-matter 拼写错、`---` 分割线 等），按既有规则处理。
     - **任何涉及内容改写、设计判断、LaTeX 化、架构重构的，一律写进待办**，绝不擅自动手——尤其是 PDF-only 笔记 / 孤儿 PDF 是否 LaTeX 化，必须给出明确的”立刻 / 低优队列 / 维持 PDF”判断 + 理由，交站主拍板。
     - 在 DAILY_REVIEW 顶部当天小节下，新建一个独立子小节 `### 🔬 抽检专项`。结构（**逐项**）：
       ```
       ### 🔬 抽检专项
       **抽检 1/10 · <类型> · `<path>`**
       - 已修复：...（无则写”无”）
       - 待办（P0/P1/P2）：...（无则写”无”）
       - 长期建议：...（LaTeX 化、架构重构等，无则写”无”）

       **抽检 2/10 · ...**
       ...
       **抽检 10/10 · ...**
       ```
     - **十项一视同仁地深审**；宁可这一节占整个 daily review 的大头、宁可写得长，也不要因为时间紧就给某一项草草几行。建设期就是要在质量门槛上多花时间。
...
8. 把本次结果写进仓库根 `DAILY_REVIEW.md`：在文件顶部插入一个 `## YYYY-MM-DD` 小节，含「✅ 本次已自动修复」「📋 待你把关（按优先级 P0/P1/P2）」「🗂 仓库卫生」「💓 后端脉搏」「📬 读者来信」「🔬 抽检专项」六部分（缺哪一节就写一行“本次无”，但不要省略小节标题）；每条说清问题、影响、建议。历史小节往下排，最多保留最近 14 天，超出删掉。确保 `_config.yml` 的 `exclude` 列表包含 `DAILY_REVIEW.md`（没有就加上）。
   - **待办前置验证（红线）**：每一条「📋 待你把关」的具体问题，写出前必须当场 `grep` / `Read` 对应文件确认状态还有这个问题；不要凭记忆、不要照搬过往 `DAILY_REVIEW.md` 里的旧条目。引用具体行号、变量名、cache 名、文件路径时，必须是**这次**看到的真实值。已被先前 commit 修过的（哪怕昨天才修）一律不写——2026-05-28 出现过把已迁完的 sw.js cache 前缀（早改为 `ruizhou03-*`）、已脱敏的 `daily_review.sh` / hooks 脚本路径、`.claude/skills/*` 里实际不存在的 zirconeey 字样误报成 P1/P2 的情况，站主在已修过的事情上反复抬头。每条待办落笔前问自己：「我刚才有没有 grep 看过这个 token 的实际行号？」
