...
#                  material_type_enum.py、filename_convention.py、maskable_icon_consistency.py、hover_no_media.py、
...
echo
echo "本次将运行：keywords + images + backend_pulse + spotcheck + material_type_enum + filename_convention + maskable_icon_consistency + hover_no_media + sibling_crosslink + bare_dollar + img_caption_md + svg_italic_zh + bare_url + frontmatter_yaml$([ "$DOW" = 1 ] && echo " + dead_links + orphan_files + pii_scan")$([ "$DOM" = 01 ] && echo " + monthly_stats")"
echo
...
echo
echo "---"
"$PY" "$SCRIPT_DIR/maskable_icon_consistency.py"
...
echo
echo "---"
"$PY" "$SCRIPT_DIR/frontmatter_yaml.py"
