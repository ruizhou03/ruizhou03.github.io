(() => {
  ...
  const GD_BUILD = '2026.06.16.7';  // 版本号：每次改动递增；刷新后看左下角徽标即可确认已加载最新版（含 AI 引擎状态）
  const SUIT_LABELS = ['♠','♥','♦','♣'];
  ...
  const SUITP = {
    ...
    0: { tr: 0.21, bl: 0.21, big: 0.66, br: 0.025, bb: 0.335, op: 1, bigH: 0.66, brH: 0.025, bbH: 0.335, opH: 1, trY: 0, blY: 0 },
    1: { tr: 0.19, bl: 0.19, big: 0.60, br: 0.04,  bb: 0.2,   op: 1, bigH: 0.60, brH: 0.04,  bbH: 0.2,   opH: 1, trY: 0, blY: 0 },
    2: { tr: 0.22, bl: 0.22, big: 0.71, br: -0.01, bb: 0.15,  op: 0.9, bigH: 0.71, brH: -0.01, bbH: 0.15, opH: 0.9, trY: 0, blY: 0 },
    3: { tr: 0.22, bl: 0.22, big: 0.66, br: 0.02,  bb: 0.34,  op: 0.9, bigH: 0.66, brH: 0.02, bbH: 0.34, opH: 0.9, trY: 0, blY: 0 },
  };
  // 角标花色「上下整体偏移」(占卡宽比例,正=下移/负=上移)：编辑器「统一」滑杆调,所有花色一起动;最终偏移 = 整体 + 逐花色 trY/blY
  const SUIT_TRY_ALL = 0;  // 右上花色(竖排手牌) 整体上下
  const SUIT_BLY_ALL = 0;  // 左下花色(横排出牌) 整体上下
