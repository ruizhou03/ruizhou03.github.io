...
(function (window) {
  ...
  function createGame(cfg) {
    ...
    function render_(now) {
      ...
      // 提示 chip 显隐
      if (hud.launchHint) {
        const anyOnPlunger = state.balls.some(b => b.onPlunger);
        const show = anyOnPlunger && plunger.charge < 0.05 && state.status !== 'gameover' && !state.paused;
        hud.launchHint.classList.toggle('show', show);
      }
    }
