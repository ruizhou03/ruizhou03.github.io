...
suﬀicient condition for the investor to have strict incentives to invest
...
The expected utility representation is convenient, but as written it
looks ad hoc — why \emph{expected} utility specifically, rather than
some other functional of the probability distribution? The agenda of
this chapter mirrors what we did under certainty: identify the axioms
on the agent's preference relation that force an expected utility
representation.
...
We make two simplifying assumptions about how the decision-maker
perceives lotteries:
...
Independence says that mixing both lotteries with a common third
lottery \(\p^3\) does not flip the ranking. Note that
\(t\p^1+(1-t)\p^3\) and \(t\p^2+(1-t)\p^3\) are formally
\emph{compound} lotteries; under the consequentialist assumption, we
identify each with the simple lottery it induces.
...
In summary, the four assumptions we maintain throughout this chapter
— completeness, transitivity, continuity, and independence — are
exactly what is needed for an expected utility representation.
...
\item
  ...
  \(\pr{\p^i}_{i = 1}^n,\pr{\q^i}_{i = 1}^n\in \P\), if
  \(\p^i\pref\q^i\) for all \(i\) and
  \(\lim_{n\to\infty}\p^i = \p^*,\,\lim_{n\to\infty}\q^i = \q^*\),
  then \(\p^*\pref\q^*\).
\item
  \textbf{Independence}: For any \(\p^1,\p^2,\p^3\in\P\) and any
  \(t\in(0,1)\),
  \[\p^1\pref\p^2\iff t\p^1+(1-t)\p^3\pref t\p^2+(1-t)\p^3.\]
\end{itemize}
}

\section{Expected Utility Representation}\label{expected-utility-representation}
...
Intuitively, a risk-averse individual has \emph{decreasing} marginal
utility — each additional dollar is worth less than the previous one.
\emph{Risk-loving} (\(u\) convex, \(F\succ\delta_{\E F}\)) and
\emph{risk-neutral} (\(u\) linear, \(F\sim\delta_{\E F}\)) attitudes are
defined symmetrically.

Risk attitudes contrast the utility of an average payoff with the
average utility of the payoffs. The \emph{certainty equivalent}
quantifies how large the gap between the two is — in monetary units.
...
Intuitively, \(c(F,u)\) is the sure amount of money the agent considers
equivalent to facing the risky lottery \(F\).
...
So far ``risk-averse'' is a yes/no property, controlled by concavity of
\(u\). The vNM expected utility is cardinally meaningful (only up to
positive affine transformations), which raises the natural follow-up
question: can we use \(u\) to \emph{quantify} how risk-averse an agent
is, and rank two agents accordingly? The two standard measures below
do exactly that.\\
...
\section{Comparison of Risky Prospects}\label{comparison-of-risky-prospects}

The coefficient of absolute risk aversion \(A(x,u)\) lets us compare
agents (or the same agent at different wealth levels). The natural
next question is about comparing \emph{lotteries}: when can we say
\(F\pref G\) for \emph{every} (risk-averse) decision-maker, regardless
of the specific shape of \(u\)?

A monetary lottery is summarized by two features that any agent will
care about: its expected payoff and its dispersion around that
expectation. This suggests two unambiguous cases in which \(F\) should
be ranked above \(G\):

\begin{enumerate}
\item
  \(F\) gives a (weakly) higher payoff than \(G\) at every probability
  level — \(F\) ``shifts the distribution to the right.''
\item
  \(F\) and \(G\) have the same expected payoff, but \(F\) is less
  dispersed.
\end{enumerate}

These two cases correspond to \emph{first-order} and
\emph{second-order} stochastic dominance respectively.

\subsection{First-Order Stochastic Dominance}\label{first-order-stochastic-dominance}
...
The first condition is the universal characterization: every
decision-maker who prefers more money to less ranks \(F\) above \(G\),
regardless of their risk attitude or the specific shape of \(u\).

\subsection{Second-Order Stochastic Dominance}\label{second-order-stochastic-dominance}
...
A mean-preserving spread keeps the expected payoff fixed but adds
noise to the realization — \(G\) is obtained from \(F\) by spreading
out the probability mass without shifting its mean.
...
\subsection{Likelihood Ratio Dominance}\label{likelihood-ratio-dominance}
...
\section{Comparative Statics Under Risk}\label{comparative-statics-under-risk}
