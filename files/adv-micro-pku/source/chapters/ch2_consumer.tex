...
Choice theory assumes that a rational decision-maker selects their most
preferred option from their choice set. Consumer theory can be viewed as
an application of choice theory. We will focus on the
\emph{preference-based approach}, with three goals in mind:

\begin{enumerate}

\item
  Formalize the ``choice set" and "preferences" of a consumer.

  \begin{itemize}
  \item
    Set of alternatives: consumption set.
  \item
    Choice set: budget set.
  \item
    Preferences: utility representation of a preference relation.
  \end{itemize}
\item
  Derive a consumer's optimal choice(s) based on the
  information of their choice set and the preference relation.
\item
  Analyze the properties of optimal choices (demand).
\end{enumerate}

\hypertarget{setups}{%
\section{Setups}\label{setups}}

At the beginning, a few assumptions are needed in consumer theory:
\asm{}{
\begin{enumerate}
...
\rmkb{
\begin{itemize}
    \item By assuming consumers to be price takers, we assume that prices \(\p\) are taken as known,
  fixed and exogenous, and there is no searching or bargaining for discounts.
    \item Linear price means that no quantity discount is offered.
    \item The idea that goods are divisible can be formally represented by the condition of \(x\in \RR_+^n\). Note that the divisibility assumption does not
  prevent us from applying the model to situations with discrete and
  indivisible goods.
\end{itemize}
}

For simplicity, we will view the entire positive quadrant as the
consumption set. 
...
The budget set differs from consumption set in that the former is additionally defined by the agent's income (and thus the price vector).
...
The preference relation is intuitive, but difficult to work with. Having
a utility representation for preferences is convenient because it transforms
the problem of preference maximization into a relatively familiar
...
Under this definition, the utility function is an \emph{ordinal}
representation. The utility level per se has no economic meaning
and is just a convenient mathematical tool to represent the
consumer's \emph{relative rankings} of different
options.
...
A natural question is that, given a preference relation \(\pref\), whether or not we
can always find a function \(u(\cdot)\) to represent \(\pref\). Luckily, in cases where preference relation is defined upon a \textit{finite} consumption set \(X\), the answer is yes.
...
\begin{itemize}
\item
  For \(|X| = n = 1\), say \(X = \{x\}\), let \(u(x) = 1\).
  . The conclusion is trivial.
\item
  Next, suppose that any preference can be represented as described for any
  set with at most \(n\) elements. We move on to consider a set \(X\) with \(n+1\) elements.

  \begin{itemize}
  \item
    Since \(C_{\pref}(X) \ne \varnothing\), the set
    \(Y = X\backslash C_\pref(X)\) has no more than \(n\) elements. By
    doing so, we can limit our discuss and deduction back to the
    hypothesized case. Thus, any preference restricted to that set can be
    represented by a utility function \(u: Y\to \{1,2,\cdots, n\}\).
  \item
...
However, if \(X\) is infinite, things are a bit more
complicated. In general, given any rational preference relation
\(\pref\) on \(\RR_+^n\), it cannot always be represented by a utility
function \(u:\RR_+^n\to \RR\).
...
Take a step forward. If a preference relation \(\pref\) is defined upon
a countable set \(X\) (possibly infinite), then \(\pref\) admits a
utility representation. We can always achieve this by construction, as
it goes in the following proposition.
...
Intuitively, the problem with the lexicographic preference relation is
that there is sudden preference reversals. For instance, we know
\((3,3)\succ (3,2)\), but \((3,2)\succ (x,3)\) for \(x\) slightly less
than \(3\). This motivates the following additional (technical)
restriction on preference relations. The ``continuity" restriction on
preferences is a condition motivated from the simple and compelling
reason that any \emph{finite} set of observed choices that is consistent
with HARP is also consistent with continuity.
...
Continuity condition implies not only that a utility representation
exists, but that a \emph{continuous} representation exists.
...
\hypertarget{utility-maximizing-problem}{%
\section{Utility Maximizing
Problem}\label{utility-maximizing-problem}}

Apart from the general problem of choice theory, what makes consumer
consumption decisions worthy of separate studies is its particular
structure that allows us to derive economically meaningful results. The
structure arises because the consumer's choice sets are
assumed to be defined by certain prices and the
consumer's income or wealth. With this in mind, when the
preference relation can be represented by a utility function
\(u(\cdot)\), the Utility Maximization Problem (UMP), or Consumer Problem (CP) is reasonably defined.
...
The idea is that, the consumer chooses a vector of goods
\(\x = (x_1,\cdots,x_n)'\ge \0\) to maximize her utility, subject to a
budget constraint that given a price vector
\(\p  =(p_1,\cdots,p_n)'\ge \0,\p\ne \0\), she cannot spend more
than her total wealth \(m\). In other words, the CP can be formulated
as:
\[C(B(\p,m);\pref) = \left\{\x\ \bigg| \ \x \text{ solves }\max_{\y\in B(\p,m) } u(\y) \right\}.\]

\hypertarget{existence-of-optimal-choices}{%
\subsection{Existence of Optimal
Choice(s)}\label{existence-of-optimal-choices}}

We are then natural to seek for the existence of an optimal choice for a
utility-maximizing consumer. Luckily, the answer is yes.
...
\hypertarget{further-assumptions}{%
\subsection{Further Assumptions}\label{further-assumptions}}

Now that existence of optimal choice is guaranteed (under rational and
continuous preference relations, and \((\p,m)\gg \0\), we still need to
answer:

\begin{itemize}
\item
  How to find the optimal choice(s)?
\item
  In particular, what (additional) restrictions on the
  consumer's preference relation can simplify our
  analysis?
\end{itemize}

\hypertarget{locally-non-satiation}{%
\paragraph{Locally Non-Satiation}\label{locally-non-satiation}}

One possible simplification is to replace the inequality in the budget
constraint with the equality: \(\p\cdot \x = m\).

Intuitively, if a consumer thinks ``more of a good is good", her
preference relation should be monotone, and she would definitely exhaust
all her wealth when maximizing her utility.
...
Indeed, the monotonicity condition proves to be a bit strong; the weaker condition of \emph{locally non-satiated}
preferences instead serves our purpose.
...
Intuitively, a preference relation is locally non-satiated if there is
no \emph{bliss} point. Moreover, a monotone preference relation must be
...
Not only do these assumptions of properties of preference relation play an important role in shaping the desired characteristics of the solutions to utility maximization problem, but each has a corresponding
property in a specific preference-representing utility function \(u\).
...
\hypertarget{marshallian-demand--indirect-utility-function}{%
\subsection{Marshallian Demand \& Indirect Utility
Function}\label{marshallian-demand--indirect-utility-function}}
...
$\x^M\pr{p,m}$ is, in general, a set of utility-maximizing consumption bundles.

Assume throughout the analysis of utility maximization problem that the consumer has a rational preference relation
\(\pref\) and makes rational decisions given \((\p,m)\gg\0\). Then
Marshallian demand and indirect utility function must satisfy the following properties.
...
\hypertarget{derivation-of-utility-maximization-problem}{%
\subsection{Derivation of Utility Maximization
Problem}\label{derivation-of-utility-maximization-problem}}

The consumer's utility maximization problem is give by:
...
\hypertarget{expenditure-minimization-problem}{%
\section{Expenditure Minimization
Problem}\label{expenditure-minimization-problem}}

In order to disentangle the price effect and the income effect, we
introduce the following expenditure minimization problem, holding the
consumer's utility at or above a certain level.
...
\rmk{
Since the consumer's utility is held at or above a certain
level, when there is a price change, \textbf{the change in the Hicksian
demand only captures the substitution effect}.
}

Similar to the analysis of the consumer's utility maximization problem, before
seeking the optimal solution(s), we approach the expenditure
minimization problem from two aspects:
...
\hypertarget{existence-of-solution}{%
...
\hypertarget{biding-utility-level}{%
\subsection{Biding Utility Level}\label{biding-utility-level}}
...
\hypertarget{summary-of-properties}{%
...
\hypertarget{duality-and-comparative-statics}{%
\section{Duality and Comparative
Statics}\label{duality-and-comparative-statics}}

\hypertarget{duality-between-ump-and-emp}{%
\subsection{Duality between UMP and
EMP}\label{duality-between-ump-and-emp}}
...
Notice that the objective function in the UMP is precisely the
constraint in the EMP and vice versa. In mathematics, the two
optimization problems are called \emph{dual problems}. By duality,
Marshallian and Hicksian demand have a special relationship.
...
Intuitively, their relationship implies that, at the
``right" level of wealth and utility, the Marshallian demand
correspondence is identical to the Hicksian demand correspondence.

\hypertarget{envelope-theorem}{%
...
Roy's identity and Shepard's lemma are
two direct applications of envelope theorem.
...
\hypertarget{slutsky-equation}{%
\subsection{Slutsky Equation}\label{slutsky-equation}}

Slutsky equation mathematically describes how Marshallian demand reacts
to a price change, decomposing into income effect and substitution
effect.
...
The interpretation for Slutsky Equation is that, the LHS represents how the Marshallian demand changes with respect to a price change, and that the first term on the RHS represents the substitution effect and the second term reveals the income effect.

Slutsky Equation allows us to develop a body of comparative statics based on it. The following definitions related to comparative statics are quite useful.

For comparative statics with regard to income $m$, we define normal good and inferior good. Intuitively, a good is normal if it loses its charm when its price increases.
...
\rmk{
Inferior good is purely a definition for goods whose demand is negatively related with the income level, but not informative of its quality.
}

Regular good and Giffen good are defined upon the comparative statics with regard to the good's own price.
...
\rmk{
A Giffen good has to be an inferior good first, because the substitution effect of a price increase will never favor the demand of that good; the income effect has to be not only positive but positive enough to counteract the negative impact of the substitution effect.
}

Another aspect of comparative statics grows in the view of analyzing a good's demand change given the price change of another good. 
...
\hypertarget{consumer-welfare}{%
\section{Consumer Welfare}\label{consumer-welfare}}

Consumer surplus is a basic quantitative measure of consumer welfare.
But here are some issues with it:

\begin{itemize}
\item
  What if more than one price would change at the same time?
\item
  No equivalent or immediate interpretation in utility theory.
\end{itemize}

To address the two shortcomings, we introduce two additional measures to
quantify changes in consumer welfare. That is, compensating variation
and equivalent variation, on measures of ``conceived wealth".
