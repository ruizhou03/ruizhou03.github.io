...
\texttt{\{r\ results=\textquotesingle{}hide\textquotesingle{},message=FALSE,warning=FALSE\}\ library(bruceR)\ library(rstatix)\ library(tidyverse)\ library(stargazer)}
...
\texttt{\{r\ warning=FALSE,message=FALSE\}\ carsales\ =\ import("car\_sales.sav")\ attach(carsales)}

\texttt{\{r\ fig.cap=\textquotesingle{}Scatter\ Plot\ of\ mpg\ and\ sales\ with\ ALL\ Observations\textquotesingle{},\ out.width=\textquotesingle{}70\%\textquotesingle{},fig.align=\textquotesingle{}center\textquotesingle{},message=FALSE,\ warning=FALSE\}\ ggplot(data\ =\ carsales,\ mapping\ =\ aes(x\ =\ mpg,y\ =\ sales))+\ \ \ geom\_point()}
...
\texttt{\{r\}\ cor(sales,mpg,use\ =\ \textquotesingle{}c\textquotesingle{})}
...
\texttt{\{r\ eval=FALSE\}\ cor.test(mpg,sales)}
...
\texttt{\{r\}\ cor(log(mpg),log(sales),use\ =\ \textquotesingle{}c\textquotesingle{})}
...
```\{r message=FALSE,warning=FALSE\} sales\_some =
sales{[}model!=``Metro''\&model!=``F-Series''{]} mpg\_some =
mpg{[}model!=``Metro''\&model!=``F-Series''{]} type\_some =
as.factor(type{[}model!=``Metro''\&model!=``F-Series''{]})

df = data.frame(sales\_some,mpg\_some,type\_some) df \%\textgreater\%
ggplot(aes(mpg\_some, sales\_some,color = type\_some)) +
geom\_point(alpha=0.8, size=0.8) + geom\_smooth(method=``lm'',se=FALSE)
+ ylab(`Sales(k)') + xlab(`MPG')
...
Finally, don't forget to \texttt{\{r\}\ detach(carsales)}
...
\texttt{\{r\ echo=FALSE,out.width=\textquotesingle{}60\%\textquotesingle{},fig.align=\textquotesingle{}center\textquotesingle{}\}\ knitr::include\_graphics("partial\ \&\ part\ correlation.png")}
...
\texttt{\{r\ echo=FALSE,out.width=\textquotesingle{}40\%\textquotesingle{},fig.align=\textquotesingle{}center\textquotesingle{}\}\ knitr::include\_graphics("MLR\ partial\ corr.jpeg")}
...
```\{r out.width=`70\%',fig.align=`center',fig.cap=`Scatter Plot of X
and Y'\} DV = c(1:8, 8:11) IV = c(4:1,8:5,12:9) covariate = c(rep(0,4),
rep(4,4), rep(7,4)) studiedData = data.frame(DV,IV,covariate)

studiedData \textbar\textgreater{} ggplot(aes(x = IV,y = DV))+
geom\_point()
...
\texttt{\{r\ message=FALSE,eval=FALSE\}\ library(ppcor)\ pcor(studiedData)}
...
\texttt{\{r\}\ \#\ example:\ a\ covariate\ can\ "make"\ a\ correlation\ that\ should\ not\ exist\ currentData\ \textless{}-\ import("health\_funding.sav")\ attach(currentData)\ studiedData\ =\ data.frame(disease,funding,visits)}

First of all, check overall zero-order correlation.

\texttt{\{r\ eval=FALSE\}\ cor(studiedData)}
...
\texttt{\{r\ eval=FALSE\}\ pcor(studiedData)\$estimate}
...
\texttt{\{r\ eval=FALSE\}\ spcor(studiedData)\$estimate}
...
\texttt{\{r\ message=FALSE\}\ currentData\ =\ import("data10-04.sav",\ sheet\ =\ 1)\ attach(currentData)\ \#\ only\ use\ the\ data\ that\ should\ be\ considered\ or\ analyzed\ studiedData\ =\ data.frame(hgrow,temp,rain,hsun,humi)}
...
\texttt{\{r\ eval=FALSE\}\ \#\ studiedData\ \%\textgreater{}\%\ dist()\ studiedData\ \ \%\textgreater{}\%\ \ \ \ scale()\ \%\textgreater{}\%\ dist()}
...
\texttt{\{r\ eval=FALSE\}\ dist\ =\ studiedData\ \ \%\textgreater{}\%\ \ \ \ scale()\ \%\textgreater{}\%\ t()\ \%\textgreater{}\%\ dist()}
...
\texttt{\{r\ eval=FALSE\}\ studiedData\ \%\textgreater{}\%\ \ \ \ as.matrix.data.frame()\ \%\textgreater{}\%\ t()\ \%\textgreater{}\%\ \ \ \ lsa::cosine()\ \%\textgreater{}\%\ \ \ \ stargazer(title\ =\ \textquotesingle{}Cosine\ Similarity\ between\ Cases\textquotesingle{})}
